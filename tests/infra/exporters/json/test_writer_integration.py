"""End-to-end test: real Vancouver pipeline → JSON export → reload from disk."""
...
class TestVancouverEndToEnd:
    ...
    async def test_records_jsonl_round_trips_through_business_licence(self, tmp_path: Path) -> None:
        _, snap_dir = await _export(tmp_path)

        lines = (snap_dir / "records.jsonl").read_text().splitlines()
        licences = [BusinessLicence.model_validate_json(line) for line in lines]
    ...
    async def test_quality_summary_reflects_known_redaction_in_fixture(
        ...
        # The middle fixture record has a REDACTED business_name.
        manifest, _ = await _export(tmp_path)
