"""Tests for the Chicago vehicle source slice."""
...
def _snapshot() -> SourceSnapshot:
    return SourceSnapshot(
        snapshot_id=SnapshotId("snap-chicago-vehicles"),
        source_id=SOURCE_ID,
...
def _map(**overrides: Any) -> CollisionVehicle:
    ...
    return ChicagoVehiclesMapper()(record, snap).record
...
class TestAdapter:
    async def test_fetches_ordered_pages_and_preserves_unit_ids(self) -> None:
...
class TestMapper:
    def test_maps_driver_vehicle_row(self) -> None:
        ...
        assert vehicle.collision_id == "crash-001"
    ...
    def test_vehicle_id_falls_back_to_crash_unit_id(self) -> None:
        vehicle = _map(vehicle_id=None, crash_unit_id="unit-fallback")

        assert vehicle.vehicle_id == "unit-fallback"

    def test_missing_vehicle_identifiers_raise_mapping_error(self) -> None:
        snap = _snapshot()
        record = RawRecord(
            snapshot_id=snap.snapshot_id,
            raw_data=_raw(vehicle_id=None, crash_unit_id=None),
            source_record_id=None,
        )

        with pytest.raises(MappingError) as excinfo:
            ChicagoVehiclesMapper()(record, snap)

        assert excinfo.value.source_record_id is None
        assert excinfo.value.source_fields == ("vehicle_id", "crash_unit_id")

    def test_pedestrian_unit_remains_representable(self) -> None:
        vehicle = _map(unit_type="PEDESTRIAN", vehicle_type="UNKNOWN/NA")
    ...
    def test_missing_occupant_count_is_not_provided(self) -> None:
        vehicle = _map(occupant_cnt=None)
...
class TestDriftAndExport:
    async def test_fixture_drift_clean_and_export_writes_records(
        ...
        async with respx.mock(assert_all_called=True) as respx_mock:
            ...
            async with httpx.AsyncClient() as client:
                pipeline_result = await run(_adapter(client), ChicagoVehiclesMapper())
                schema_obs = SchemaObserver(spec=CHICAGO_VEHICLES_SCHEMA)
                taxonomy_obs = TaxonomyObserver(specs=CHICAGO_VEHICLES_TAXONOMIES)
