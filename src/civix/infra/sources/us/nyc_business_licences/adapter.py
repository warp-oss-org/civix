...
class NycBusinessLicencesAdapter:
    """Fetches NYC DCWP premises business licences over Socrata SODA."""
    ...
    async def _fetch_total_count(self) -> int:
        ...
        try:
            response = await self.client.get(
                url,
                params={"$select": "count(*)", "$where": PREMISES_FILTER},
    ...
    def _build_record(self, *, snapshot_id: SnapshotId, row: Any) -> RawRecord:
        ...
        source_record_id = raw_data.get("license_nbr")
    ...
    def _build_snapshot(self, *, fetched_at: datetime, record_count: int) -> SourceSnapshot:
        ...
        return SourceSnapshot(
            ...
            fetch_params={"$where": PREMISES_FILTER},
        )
