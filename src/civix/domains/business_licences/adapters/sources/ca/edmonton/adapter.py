...
dataset. The adapter makes a count probe with SoQL, then walks record
pages with `$limit`/`$offset`. Socrata-computed region fields are
transport artifacts and are stripped before records enter Civix
snapshots.
"""
...
class EdmontonBusinessLicencesAdapter:
    """Fetches Edmonton's business-licences dataset over Socrata SODA."""
    ...
    def _build_record(self, *, snapshot_id: SnapshotId, row: Any) -> RawRecord:
        ...
        source_record_id = raw_data.get("externalid")
