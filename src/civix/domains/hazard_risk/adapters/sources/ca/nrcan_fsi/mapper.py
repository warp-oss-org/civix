"""Natural Resources Canada Flood Susceptibility Index mappers."""
...
AREA_MAPPER_ID: Final[MapperId] = MapperId("nrcan-fsi-area")
SCORES_MAPPER_ID: Final[MapperId] = MapperId("nrcan-fsi-scores")
AREA_MAPPER_VERSION: Final[str] = "0.1.0"
SCORES_MAPPER_VERSION: Final[str] = "0.1.0"
METHODOLOGY_LABEL: Final[str] = "Natural Resources Canada Flood Susceptibility Index"

_AREA_IDENTIFIER_TAXONOMY_ID: Final[str] = "nrcan-fsi-area-identifier"
_AREA_KIND_TAXONOMY_ID: Final[str] = "nrcan-fsi-area-kind"
_HAZARD_TAXONOMY_ID: Final[str] = "nrcan-fsi-hazard"
_RATING_TAXONOMY_ID: Final[str] = "nrcan-fsi-rating"
_SCORE_FIELD_TAXONOMY_ID: Final[str] = "nrcan-fsi-score-field"
_SCORE_UNIT_TAXONOMY_ID: Final[str] = "nrcan-fsi-score-unit"
_SCORE_SCALE_SOURCE_FIELD: Final[str] = "source.methodology.score_scale"


@dataclass(frozen=True, slots=True)
class NrcanFsiAreaMapper:
    """Maps one NRCan FSI row to a hazard-risk area."""
    ...
    def __call__(self, record: RawRecord, snapshot: SourceSnapshot) -> MapResult[HazardRiskArea]:
        _require_fsi_snapshot(snapshot, self.version, record)
        raw = record.raw_data
        cell_id = _required_text(raw, "cell_id", self.version, record)
        area = HazardRiskArea(
            ...
            area_kind=MappedField(
                ...
                source_fields=("cell_id",),
            ),
            source_area_kind=MappedField(
                value=_category("flood-susceptibility-cell", taxonomy_id=_AREA_KIND_TAXONOMY_ID),
                quality=FieldQuality.STANDARDIZED,
                source_fields=("cell_id",),
            ),
            name=MappedField(
                value=_required_text(raw, "community_name", self.version, record),
                quality=FieldQuality.DIRECT,
                source_fields=("community_name",),
            ),
            jurisdiction=MappedField(
                value=Jurisdiction(
                    country="CA",
                    region=_required_text(raw, "province_code", self.version, record),
                ),
                quality=FieldQuality.STANDARDIZED,
                source_fields=("province_code",),
            ),
            administrative_areas=MappedField(
                value=(
                    _required_text(raw, "province_code", self.version, record),
                    _required_text(raw, "community_name", self.version, record),
                ),
                quality=FieldQuality.DIRECT,
                source_fields=("province_code", "community_name"),
            ),
...
@dataclass(frozen=True, slots=True)
class NrcanFsiScoresMapper:
    """Maps one NRCan FSI row to susceptibility score facts."""
...
def _map_numeric_score(
    *,
    ...
    try:
        ...
        raise MappingError(
            "invalid NRCan FSI susceptibility score",
            mapper=mapper,
    ...
    return _build_score(
        ...
        score_scale=MappedField(
            value=ScoreScale(minimum=Decimal("0"), maximum=Decimal("1")),
            quality=FieldQuality.STANDARDIZED,
    ...
    *,
    ...
    return HazardRiskScore(
        ...
        methodology_url=MappedField(
            value=_required_text(raw, "methodology_url", mapper, record),
            quality=FieldQuality.DIRECT,
            source_fields=("methodology_url",),
        ),
...
def _require_fsi_snapshot(
    snapshot: SourceSnapshot,
    mapper: MapperVersion,
    record: RawRecord,
) -> None:
    if snapshot.dataset_id == NRCAN_FSI_DATASET_ID:
        return

    raise MappingError(
        "NRCan FSI mapper requires the Flood Susceptibility Index dataset",
        mapper=mapper,
        source_record_id=record.source_record_id,
...
def _map_area_identifiers(
    ...
    fields = ("cell_id", "community_id", "province_code")

    return MappedField(
        value=tuple(
            SourceIdentifier(
                value=_required_text(raw, field_name, mapper, record),
                identifier_kind=_category(field_name, taxonomy_id=_AREA_IDENTIFIER_TAXONOMY_ID),
            )
...
def _map_geometry_ref(
    ...
    return MappedField(
        value=GeometryRef(
            ...
            query_keys=(("cell_id", _required_text(raw, "cell_id", mapper, record)),),
        ),
        quality=FieldQuality.STANDARDIZED,
        source_fields=("geometry_uri", "geometry_layer", "geometry_id", "source_crs", "cell_id"),
    )
...
def _source_hazard(raw: Mapping[str, Any], mapper: MapperVersion, record: RawRecord) -> CategoryRef:
    mechanism = _required_text(raw, "flood_mechanism", mapper, record)

    return _category(mechanism, taxonomy_id=_HAZARD_TAXONOMY_ID, label=mechanism)


def _map_source_caveats() -> MappedField[tuple[CategoryRef, ...]]:
    return MappedField(
        value=nrcan_fsi_caveat_categories(),
        quality=FieldQuality.STANDARDIZED,
        source_fields=(NRCAN_FSI_METADATA_SOURCE_FIELD,),
    )
...
def _category(code: str, *, taxonomy_id: str, label: str | None = None) -> CategoryRef:
    return CategoryRef(
        ...
        taxonomy_version=NRCAN_FSI_TAXONOMY_VERSION,
    )
